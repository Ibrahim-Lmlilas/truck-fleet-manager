...
const calculerEcheances = async (req, res, next) => {
    try {
        const { vehiculeId } = req.params;
        const { typeMaintenance, intervalleKm, intervalleJours } = req.query;

        const camion = await Camion.findById(vehiculeId);
        if (!camion) {
            return res.status(404).json({
                success: false,
                message: 'Véhicule non trouvé'
        ...
        // Trouver la dernière maintenance effectuée de ce type
        ...
        if (intervalleKm) {
            const kmIntervalle = parseInt(intervalleKm);
            const kmDerniereMaintenance = derniereMaintenance?.kmMaintenance || camion.derniereMaintenanceKm || 0;
            const kmActuel = camion.kilometrage;
            
            echeanceKm = kmDerniereMaintenance + kmIntervalle;
            kmRestant = echeanceKm - kmActuel;
            estEnRetard = kmRestant < 0;
        }

        if (intervalleJours) {
            const joursIntervalle = parseInt(intervalleJours);
            const dateDerniereMaintenance = derniereMaintenance?.dateFait || camion.updatedAt || new Date();
            const dateActuelle = new Date();
            
            echeanceDate = new Date(dateDerniereMaintenance);
            echeanceDate.setDate(echeanceDate.getDate() + joursIntervalle);
            
            const diffTime = echeanceDate - dateActuelle;
            joursRestants = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
            estEnRetard = joursRestants < 0;
        }

        // Si les deux intervalles sont fournis, utiliser la première échéance
        let prochaineEcheance = null;
        if (echeanceKm && echeanceDate) {
            // Estimer la date d'échéance km basée sur la consommation moyenne
            const kmRestantPourEcheance = echeanceKm - camion.kilometrage;
            // Estimation: 100 km par jour (peut être ajusté)
            const joursEstimes = Math.ceil(kmRestantPourEcheance / 100);
            const dateEcheanceKm = new Date();
            dateEcheanceKm.setDate(dateEcheanceKm.getDate() + joursEstimes);
            
            prochaineEcheance = dateEcheanceKm < echeanceDate ? 
                { type: 'km', date: dateEcheanceKm, km: echeanceKm } :
                { type: 'date', date: echeanceDate, km: null };
        } else if (echeanceKm) {
            const kmRestantPourEcheance = echeanceKm - camion.kilometrage;
            const joursEstimes = Math.ceil(kmRestantPourEcheance / 100);
            const dateEcheanceKm = new Date();
            dateEcheanceKm.setDate(dateEcheanceKm.getDate() + joursEstimes);
            prochaineEcheance = { type: 'km', date: dateEcheanceKm, km: echeanceKm };
        } else if (echeanceDate) {
            prochaineEcheance = { type: 'date', date: echeanceDate, km: null };
        }
        ...
        res.status(200).json({
            ...
            data: {
                vehicule: {
                    ...
                    kilometrageActuel: camion.kilometrage
                },
                typeMaintenance: typeMaintenance || 'tous',
                derniereMaintenance: derniereMaintenance ? {
                    date: derniereMaintenance.dateFait,
                    km: derniereMaintenance.kmMaintenance
                } : null,
                echeances: {
                    km: echeanceKm ? {
                        valeur: echeanceKm,
